...
wandb.init(project="Shubhodeeo_Final_CS6190_DeepLearing_Assignment1",name = "Question 1")
...
class Initializer :
    ...
    def Initialize(self,hidden_layers,npl):
        ...
        W.append(np.zeros((npl,784)))           #,dtype=np.float128)) input layer h0

        for _ in range(hidden_layers-1):
            W.append(np.zeros((npl,npl)))       #,dtype=np.float128

        W.append(np.zeros((10,npl)))            #,dtype=np.float128

        b = [[]]    # list consiting of all the b's

        # Create and append each bi vector filled with zeros to the list
        b.append(np.zeros(npl))       #,dtype=np.float128

        for _ in range(hidden_layers-1):
            b.append(np.zeros(npl))   #,dtype=np.float128

        b.append(np.zeros(10))        #,dtype=np.float128
    ...
    def Initialize2(self,hidden_layers,npl):
          ...
          # Create and append each Wi matrix filled with random values to the list
            W.append(np.random.randn(npl, 784))         # input layer h0 .astype(np.float128)
            for _ in range(hidden_layers-1):
                W.append(np.random.randn(npl, npl))     #.astype(np.float128)

            W.append(np.random.randn(10, npl))      #.astype(np.float128)

            b = [[]]    # list consiting of all the b's

            # Create and append each bi vector filled with random values to the list
            b.append(np.random.randn(npl))          #.astype(np.float128)

            for _ in range(hidden_layers-1):
                b.append(np.random.randn(npl))      #.astype(np.float128)
                
            b.append(np.random.randn(10))       #.astype(np.float128)
            return W,b
    ...
    def XavierIntializer(self,hidden_layers,npl):
        ...
        def xavier_init(n_in, n_out):
            return np.random.randn(n_out, n_in)* np.sqrt(1 / n_in)      #.astype(np.float128)
...
class Gradient_descent :
    ...
    def backward_propagation(self,A,H,W,b,y):
        ...
        for k in range(L,0,-1):
            #print(delW[k].shape)
            #print(delA[k].shape,H[k-1].shape)
            delW[k] = np.matmul(delA[k],H[k-1].T) #+ (self.config['regularization']*W[k])
            ...
            if k>1 :
                ...
                # print(delb[k].shape,delh[k-1].shape,diff_vect.shape,delA[k-1].shape)
    ...
    def forward_propagation(self,W,b,layers,inpl):
        ...
        #print(b[layers].shape,W[layers].shape,H[layers-1].shape,i)
    ...
    def Stocastic_Gradient_descent(self) :
        ...
        for i in range(epochs):
            ...
            print(training_accuracy[-1],training_loss[-1]) 
            print(validation_loss[-1],validation_accuracy[-1]) 
    ...
    def NAG_descent(self) :    # function(layers ,neurons per layer)
        ...
        for i in range(epochs):
            ...
            for j in range(len(x_train)):
                h0 = x_train[j]    
                A,H = self.forward_propagation(tempW,tempb,self.layers,h0)
                y = y_train[j]

                if self.config['loss'] == 'cross_entropy' :
                    s = s - math.log(y@H[self.layers] + 1e-20)
                else :
                    s = s + np.sum((y-H[self.layers])**2)
                
                if np.argmax(y) == np.argmax(H[self.layers]) :
                    c = c + 1
                ...
                if((j+1)%self.batch == 0):
                    for k in range(1,len(prev_vW)):
                        vW[k] = beta*prev_vW[k] + self.eta*delW[k]
                        vb[k] = beta*prev_vb[k] + self.eta*delb[k]
    ...
    def RMSprop(self) :
        ...
        for i in range(epochs):
            ...
            s = 0.0
            c = 0
            for j in range(0,len(x_train)//self.batch): #len(trainX)
                h0 = x_train[j*self.batch : (j+1)*self.batch]
    ...
    def Run_Models(self):
        run_name = "op_{}_ep_{}_npl_{}_eta_{}_bs_{}_ini_{}_reg_{}_loss{}_activ{}".format(self.config['optimizer'],self.config['epochs'],self.config['neurons_per_layer'],self.config['learning_rate'],self.config['batch_size'],self.config['Initialization'],self.config['regularization'],self.config['loss'],self.config['activation'])
